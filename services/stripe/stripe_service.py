        ...
        line_items.append(
            {
                ...
                "price_data": {
                    "currency": currency,
                    "unit_amount": _to_minor_units(item["unitPrice"]),
...
async def initiate_checkout_session(
    ...
    try:
        if not settings.STRIPE_SECRET_KEY:
            return _error_response("Stripe secret key is not configured")

        if not settings.STRIPE_CHECKOUT_SUCCESS_URL or not settings.STRIPE_CHECKOUT_CANCEL_URL:
            return _error_response("Stripe checkout URLs are not configured")

        currency = settings.STRIPE_CURRENCY.strip().lower()
        if len(currency) != 3:
            return _error_response("Invalid Stripe currency configuration")
        ...
        if not checkout_session_id or not checkout_url:
            return _error_response("Stripe returned an invalid checkout session")
